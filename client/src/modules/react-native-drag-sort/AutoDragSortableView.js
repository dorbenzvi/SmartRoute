...
class AutoDragSortableView extends Component {
  constructor(props) {
    ...
    for (let index = 0; index < props.dataSource.length; index++) {
      var temp = props.dataSource.find((item) => item.StopOrder == index + 1)
      stops.push(temp)
    }

    const dataSource = stops.map((item, index) => {
      const newData = {}
  ...
  componentDidMount() {
    this.props.setRefToField(
      'arrayToggle' + this.props.routeToView,
      this.startTouch
    )
    this.props.setRefToField('toggle', this)
    this.initTag()
  ...
  //ohad
  toggle(indexToExpense, from) {
    if (from != 0) {
      this.props.FieldsRef['Stop' + (indexToExpense + 1)].toggleExpand()
    }

    const itemWidth =
      this.props.childrenWidth +
      this.props.marginChildrenLeft +
      this.props.marginChildrenRight
    const itemHeight =
      this.props.childrenHeight +
      this.props.marginChildrenTop +
      this.props.marginChildrenBottom

    var counter = 0
    // this.reComplexDataSource(true,props) // react < 16.3
    // react > 16.3 Fiber
    const rowNum = parseInt(this.props.parentWidth / itemWidth)
    this.props.setRefToField('indexArray', this.indexArray)
    if (this.indexArray.includes(parseInt(indexToExpense))) {
      this.indexArray = this.indexArray.filter(
        (item) => parseInt(item) != parseInt(indexToExpense)
      )
    } else this.indexArray.push(parseInt(indexToExpense))

    const dataSource = this.state.dataSource.map((item, index) => {
      const additionalSpace = 63

      const left = (index % rowNum) * itemWidth
      const top =
        parseInt(index / rowNum) * itemHeight + additionalSpace * counter

      if (this.indexArray.includes(parseInt(index))) {
        counter++
      }
      const newData = { ...this.state.dataSource[index] }
      //newData.data = item
      newData.originIndex = index
      newData.originLeft = left
      newData.originTop = top
    ...
    this.setState({
      dataSource: dataSource,
      height:
        Math.ceil(dataSource.length / rowNum) * itemHeight +
        this.indexArray.length * 63,
    })
  }
  ...
  startTouch(touchIndex) {
    ...
    //   // this.props.FieldsRef['Stop' + (this.indexArray[index] + 1)].toggleExpand()
    // }
    ...
    const dataSource = this.state.dataSource.map((item, index) => {
      const newData = {}
      const left = (index % this.rowNum) * this.itemWidth
      const top = parseInt(index / this.rowNum) * this.itemHeight
    ...
    this.state = {
      dataSource: dataSource,
      curPropsDataSource: this.props.dataSource,
      height: Math.ceil(dataSource.length / this.rowNum) * this.itemHeight,
    }
      ...
      return (
        ...
        >
          <TouchableOpacity
            ...
            onPress={() => {
              this.props.FieldsRef['Stop' + (index + 1)].toggleExpand()
