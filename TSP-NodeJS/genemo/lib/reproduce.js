...
const reproduce = options => {
  ...
  return (evaluatedPopulation, random, collectLog) => {
    ...
    timer.start();
    const childrenPairs = R.range(0, Math.ceil(targetPopulationSize / 2)).map(() => {
    ...
    timer.start();
    const mutatedPopulation = newPopulation.map(individual => random() <= mutationProbability ? mutate(individual, random) : individual);
