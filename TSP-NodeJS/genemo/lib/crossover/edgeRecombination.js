...
const getNeighbors = (array, index) => {
  const neighborAIndex = index === 0 ? array.length - 1 : index - 1;
  const neighborBIndex = (index + 1) % array.length;
  return [...new Set([neighborAIndex, neighborBIndex])].filter(neighborIndex => neighborIndex !== index).map(neighborIndex => array[neighborIndex]);
};
...
const insertNeighborsIntoMap = (map, individual, hashGene) => new Map(individual.map((gene, index) => {
  const hash = hashGene(gene);
  const exisitngNeighbors = map.get(hash) || [];
  return [hash, mergeGenesLists(exisitngNeighbors, getNeighbors(individual, index), hashGene)];
}));
...
const appendRemainingGenesToChild = (child, currentGene, neighborsMap, parent, hashGene, randomItem) => {
  if (child.length >= parent.length) {
    return child;
  }

  const currentGeneHash = hashGene(currentGene);
  const newChild = [...child, currentGene]; // Remove currentGene from neighbor lists

  const newNeighborsMap = new Map([...neighborsMap.entries()].map(([hash, neighbors]) => [hash, neighbors.filter(neighbor => hashGene(neighbor) !== currentGeneHash)]));
  ...
  const nextGene = hasCurrentGeneAnyNeighbors ? (() => {
    ...
    const genesNeighbors = currentGeneNeighbors.map(gene => [gene, newNeighborsMap.get(hashGene(gene))]);
    const minNeighborsNumber = min(genesNeighbors.map(([, neighbors]) => neighbors.length));
    const genesWithLeastNeighbors = genesNeighbors.filter(([, neighbors]) => neighbors.length === minNeighborsNumber).map(([gene]) => gene);
    return randomItem(genesWithLeastNeighbors);
  })() : (() => {
    const nodesNotInChild = parent.filter(gene => !newChild.some(childGene => hashGene(gene) === hashGene(childGene)));
    return randomItem(nodesNotInChild);
  })();
  return appendRemainingGenesToChild(newChild, nextGene, newNeighborsMap, parent, hashGene, randomItem);
};
...
const createSingleChild = ([mother, father], hashGene, random) => {
  ...
  return appendRemainingGenesToChild(child, currentGene, neighborsMap, mother, hashGene, randomItem);
};
