...
const createChildUsingPMXCrossover = ([parent1, parent2], minIndex, maxIndex) => {
  ...
  const child = new Array(individualLength).fill(); // Copy random swath from parent1
  ...
  const mapping = new Map([...R.range(minIndex, maxIndex + 1).map(index => [parent2[index], parent1[index]]), ...R.range(minIndex, maxIndex + 1).map(index => [parent1[index], parent2[index]])]); // In empty places insert elements based on mapping
