...
const mergeFitnessValuesWithPopulation = (population, fitnessValues) => R.zip(population, fitnessValues).map(([individual, fitness]) => ({
  individual,
  fitness
}));
/**
...
const run = async options => {
  ...
  const mainLoopBody = async ({
    ...
    const childrenPopulation = await reproduce(parentsPopulation, random, collectReproduceLog);
    logsCollector.collectClockValue('reproduce');
    logsCollector.startClock('evaluatePopulation');
    const evaluatedChildrenPopulation = mergeFitnessValuesWithPopulation(childrenPopulation, await evaluatePopulation(childrenPopulation, random));
  ...
  let evaluatedPopulation = mergeFitnessValuesWithPopulation(initialPopulation, await evaluatePopulation(initialPopulation, random)); // eslint-disable-next-line no-constant-condition

  while (true) {
    logsCollector.startClock('lastIteration');
    iteration += 1;
    evaluatedPopulation = await executeMainLoopBody({
      evaluatedPopulation
    });
    ...
    const iterationData = {
      ...
      getLowestFitnessIndividual: () => {
        ...
        const lowestFitness = min(evaluatedPopulation.map(({
          fitness
        
        }) => fitness));
        const evaluatedIndividualWithLowestFitness = evaluatedPopulation.find(({
          fitness
      ...
      getHighestFitnessIndividual: () => {
        const highestFitness = max(evaluatedPopulation.map(({
          fitness
        }) => fitness));
        const evaluatedIndividualWithHighestFitness = evaluatedPopulation.find(({
          fitness
